...
impl PersistedEventPayload {
    fn from_proto(payload: &apkw_proto::apkw::v1::job_event::Payload) -> Self {
        match payload {
            apkw_proto::apkw::v1::job_event::Payload::StateChanged(state) => {
                PersistedEventPayload::StateChanged {
                    new_state: state.new_state,
                }
            }
            apkw_proto::apkw::v1::job_event::Payload::Progress(progress) => {
            ...
            apkw_proto::apkw::v1::job_event::Payload::Log(log) => PersistedEventPayload::Log {
                chunk: log.chunk.as_ref().map(PersistedLogChunk::from_proto),
            },
            apkw_proto::apkw::v1::job_event::Payload::Completed(done) => {
            ...
            apkw_proto::apkw::v1::job_event::Payload::Failed(failed) => {
    ...
    fn into_proto(self) -> apkw_proto::apkw::v1::job_event::Payload {
        match self {
            PersistedEventPayload::StateChanged { new_state } => {
                apkw_proto::apkw::v1::job_event::Payload::StateChanged(JobStateChanged {
                    new_state,
                })
            }
            PersistedEventPayload::Progress { progress } => {
                apkw_proto::apkw::v1::job_event::Payload::Progress(JobProgressUpdated {
                    progress: progress.map(PersistedJobProgress::into_proto),
                })
            }
            PersistedEventPayload::Log { chunk } => {
                apkw_proto::apkw::v1::job_event::Payload::Log(JobLogAppended {
                    chunk: chunk.map(PersistedLogChunk::into_proto),
                })
            }
            PersistedEventPayload::Completed { summary, outputs } => {
                apkw_proto::apkw::v1::job_event::Payload::Completed(JobCompleted {
                    summary,
                    outputs: outputs
                        .into_iter()
                        .map(KeyValueRecord::into_proto)
            ...
            PersistedEventPayload::Failed { error } => {
                apkw_proto::apkw::v1::job_event::Payload::Failed(JobFailed {
...
fn event_kind(event: &JobEvent) -> JobEventKind {
    match event.payload {
        Some(apkw_proto::apkw::v1::job_event::Payload::StateChanged(_)) => {
            JobEventKind::StateChanged
        }
        Some(apkw_proto::apkw::v1::job_event::Payload::Progress(_)) => JobEventKind::Progress,
        Some(apkw_proto::apkw::v1::job_event::Payload::Log(_)) => JobEventKind::Log,
        Some(apkw_proto::apkw::v1::job_event::Payload::Completed(_)) => JobEventKind::Completed,
        Some(apkw_proto::apkw::v1::job_event::Payload::Failed(_)) => JobEventKind::Failed,
...
fn run_stream_config(req: &StreamRunEventsRequest) -> RunStreamConfig {
    let buffer_max = if req.buffer_max_events == 0 {
        read_env_usize("APKW_RUN_STREAM_BUFFER_MAX", DEFAULT_RUN_STREAM_BUFFER_MAX)
    } else {
        req.buffer_max_events as usize
    };
    let max_delay_ms = if req.max_delay_ms == 0 {
        read_env_u64(
            "APKW_RUN_STREAM_MAX_DELAY_MS",
    ...
    let discovery_ms = if req.discovery_interval_ms == 0 {
        read_env_u64(
            "APKW_RUN_STREAM_DISCOVERY_MS",
            DEFAULT_RUN_STREAM_DISCOVERY_MS,
        )
    } else {
        req.discovery_interval_ms
    };
    let flush_ms = read_env_u64("APKW_RUN_STREAM_FLUSH_MS", DEFAULT_RUN_STREAM_FLUSH_MS);
    RunStreamConfig {
...
fn state_file_path() -> PathBuf {
    apkw_util::state_file_path(STATE_FILE_NAME)
}
...
fn retention_policy_from_env() -> RetentionPolicy {
    let retention_days = read_env_u64(
        "APKW_JOB_HISTORY_RETENTION_DAYS",
        DEFAULT_JOB_HISTORY_RETENTION_DAYS,
    );
    let max_jobs = read_env_usize("APKW_JOB_HISTORY_MAX", DEFAULT_JOB_HISTORY_MAX);
...
fn mk_event(job_id: &str, payload: apkw_proto::apkw::v1::job_event::Payload) -> JobEvent {
    JobEvent {
        at: Some(now_ts()),
        job_id: Some(Id {
            value: job_id.to_string(),
...
impl JobSvc {
    ...
    async fn publish(&self, job_id: &str, payload: apkw_proto::apkw::v1::job_event::Payload) {
        if let Some(rec) = self.store.get(job_id).await {
...
impl JobService for JobSvc {
    ...
    async fn publish_job_event(
        ...
        match &payload {
            ...
            apkw_proto::apkw::v1::job_event::Payload::Completed(_) => {
                self.update_state_only(&job_id, JobState::Success).await;
            }
            apkw_proto::apkw::v1::job_event::Payload::Failed(_) => {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    init_tracing()?;
    init_service_telemetry("apkw-core", env!("CARGO_PKG_VERSION"), "job");
