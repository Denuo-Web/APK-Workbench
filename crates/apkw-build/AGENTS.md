# BuildService Agent Notes (apkw-build)
...
- Implementation lives in crates/apkw-build/src/main.rs with a tonic server; shared bootstrap and
...
- Persist build/job artifact records in ~/.local/share/apkw/state/builds.json.
...
- ReloadState reloads persisted build state from ~/.local/share/apkw/state/builds.json on demand.
...
## Environment / config
- APKW_BUILD_ADDR sets the bind address (default 127.0.0.1:50054).
- APKW_JOB_ADDR sets the JobService address.
- APKW_PROJECT_ADDR sets the ProjectService address.
- APKW_OBSERVE_ADDR sets the ObserveService address for run output upserts.
- APKW_GRADLE_DAEMON and APKW_GRADLE_STACKTRACE control Gradle flags.
- APKW_GRADLE_REQUIRE_WRAPPER forces the Gradle wrapper to be present.
- APKW_GRADLE_USER_HOME overrides GRADLE_USER_HOME for builds.
