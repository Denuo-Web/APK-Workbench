...
- Entry point and GTK wiring live in `crates/apkw-ui/src/main.rs`, with modules split into
...
- AppConfig holds service addresses and pulls defaults from env:
  APKW_JOB_ADDR, APKW_TOOLCHAIN_ADDR, APKW_PROJECT_ADDR, APKW_BUILD_ADDR,
  APKW_TARGETS_ADDR, APKW_OBSERVE_ADDR, APKW_WORKFLOW_ADDR.
- AppConfig persists to `~/.local/share/apkw/state/ui-config.json` with service endpoints, active context, last job selections, and telemetry opt-in.
- Job log export helpers (default paths + history fetch) and shared data-dir utilities come from `apkw-util`.
...
- The Targets page includes an "Open Cuttlefish Env" button using APKW_CUTTLEFISH_ENV_URL (default https://localhost:1443).
...
(Clients list includes UI and CLI items; some references below point to crates/apkw-cli.)
