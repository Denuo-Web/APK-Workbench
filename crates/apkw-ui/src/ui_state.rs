...
impl Default for TargetsState {
    fn default() -> Self {
        ...
        let cuttlefish_branch = std::env::var("APKW_CUTTLEFISH_BRANCH").unwrap_or_default();
        let cuttlefish_target = std::env::var("APKW_CUTTLEFISH_TARGET").unwrap_or_default();
        let cuttlefish_build_id = std::env::var("APKW_CUTTLEFISH_BUILD_ID").unwrap_or_default();
        Self {
            log: String::new(),
            use_job_id: false,
            job_id: String::new(),
...
impl UiState {
    ...
    pub(crate) fn save(&self) -> io::Result<()> {
        ...
        apkw_util::write_json_atomic(&ui_state_path(), &trimmed)
    }
