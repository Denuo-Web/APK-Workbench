...
const GRADLE_MODEL_START: &str = "APKW_MODEL_START";
const GRADLE_MODEL_END: &str = "APKW_MODEL_END";
...
fn gradle_daemon_enabled() -> bool {
    matches!(
        std::env::var("APKW_GRADLE_DAEMON"),
...
fn gradle_stacktrace_enabled() -> bool {
    matches!(
        std::env::var("APKW_GRADLE_STACKTRACE"),
        Ok(val) if val == "1" || val.eq_ignore_ascii_case("true")
    )
}
...
fn gradle_wrapper_required() -> bool {
    matches!(
        std::env::var("APKW_GRADLE_REQUIRE_WRAPPER"),
        Ok(val) if val == "1" || val.eq_ignore_ascii_case("true")
    )
}
...
fn gradle_user_home() -> Option<PathBuf> {
    ...
    if let Ok(configured) = std::env::var("APKW_GRADLE_USER_HOME") {
...
gradle.projectsEvaluated {
    ...
    println("APKW_MODEL_START")
    println(JsonOutput.toJson(model))
    println("APKW_MODEL_END")
}
"#
}
...
fn spawn_gradle(project_dir: &Path, args: &[String]) -> Result<GradleSpawn, Status> {
    ...
    let (mut cmd, description) = if wrapper.is_file() {
        ...
        if gradle_wrapper_required() {
            return Err(Status::failed_precondition(
                "gradle wrapper not found (APKW_GRADLE_REQUIRE_WRAPPER=1)",
            ));
        }
...
async fn load_gradle_model(
    ...
    let script_path =
        std::env::temp_dir().join(format!("apkw-gradle-model-{}.gradle", now_millis()));
