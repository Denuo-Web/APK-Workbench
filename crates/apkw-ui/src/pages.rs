...
use apkw_util::{data_dir, state_export_path};
...
fn is_apkw_project_dir(path: &str) -> bool {
    let path = path.trim();
    if path.is_empty() {
        return false;
    }
...
fn queue_project_open(
    ...
    let path = path.trim();
    if path.is_empty() || !is_apkw_project_dir(path) {
        return false;
    }
...
fn select_zip_dialog(
    ...
    let filter = gtk::FileFilter::new();
    filter.set_name(Some("APKW State Archive (.zip)"));
...
pub(crate) fn page_targets(
    ...
    let default_branch = std::env::var("APKW_CUTTLEFISH_BRANCH").unwrap_or_default();
...
pub(crate) fn page_settings(
    ...
    let telemetry_dir = data_dir().join("telemetry").join("apkw-ui");
