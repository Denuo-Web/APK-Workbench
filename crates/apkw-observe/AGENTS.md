# ObserveService Agent Notes (apkw-observe)
...
## gRPC contract
- proto/apkw/v1/observe.proto
- RPCs: ListRuns, ListRunOutputs, ExportSupportBundle, ExportEvidenceBundle, UpsertRun, UpsertRunOutputs, ReloadState

## Current implementation details
- Implementation lives in crates/apkw-observe/src/main.rs with a tonic server; shared bootstrap
  and path/time helpers come from `apkw-util`.
- Run history and output inventory are persisted in ~/.local/share/apkw/state/observe.json and paged in list_runs/list_run_outputs.
...
- export_support_bundle creates a JobService job, writes a zip bundle under
  ~/.local/share/apkw/bundles, and streams progress/log events.
...
- Bundle contents include a manifest, config snapshots (env + state files), optional toolchain
  ...
  (`~/.local/share/apkw/state/jobs.json`).
...
## Environment / config
- APKW_OBSERVE_ADDR sets the bind address (default 127.0.0.1:50056).
- APKW_OBSERVE_BUNDLE_RETENTION_DAYS controls bundle age retention (default 30, 0 disables).
- APKW_OBSERVE_BUNDLE_MAX controls max bundle count (default 50, 0 disables).
- APKW_OBSERVE_TMP_RETENTION_HOURS controls tmp directory retention (default 24, 0 disables).
