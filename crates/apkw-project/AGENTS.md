# ProjectService Agent Notes (apkw-project)
...
- Implementation lives in crates/apkw-project/src/main.rs with a tonic server; shared bootstrap
  and path/time helpers come from `apkw-util`.
- State is persisted in ~/.local/share/apkw/state/projects.json and cached in-memory.
- list_templates loads a JSON registry from APKW_PROJECT_TEMPLATES (or templates/registry.json),
...
- create_project copies files from the template directory, streams job progress/logs to
  JobService, and persists metadata in .apkw/project.json plus the recent list; requests can
...
- create_project attempts to write local.properties with sdk.dir from ANDROID_SDK_ROOT/ANDROID_HOME
  or the installed APKW SDK toolchain state, logging when it skips or overwrites nothing.
...
- open_project reads .apkw/project.json when present, otherwise generates metadata and persists it.
...
## Environment / config
- APKW_PROJECT_ADDR sets the bind address (default 127.0.0.1:50053).
