...
#[derive(Parser)]
#[command(name = "apkw-cli", version, about = "APK Workbench CLI")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
#[derive(Subcommand)]
enum StateCmd {
    /// Save local APKW state to a zip archive
    Save {
        #[arg(long, default_value_t = default_job_addr())]
        job_addr: String,
        /// Output path for the archive (defaults to ~/.local/share/apkw/state-exports)
        #[arg(long)]
        output: Option<String>,
        #[arg(long)]
    ...
    /// Open a local APKW state archive and reload services
    Open {
        #[arg(long, default_value_t = default_job_addr())]
        job_addr: String,
        #[arg(long, default_value_t = default_toolchain_addr())]
...
fn default_job_addr() -> String {
    if let Ok(val) = std::env::var("APKW_JOB_ADDR") {
        return val;
    }
...
fn default_toolchain_addr() -> String {
    if let Ok(val) = std::env::var("APKW_TOOLCHAIN_ADDR") {
        return val;
    }
    let cfg = load_cli_config();
...
fn default_targets_addr() -> String {
    if let Ok(val) = std::env::var("APKW_TARGETS_ADDR") {
        return val;
    }
    let cfg = load_cli_config();
...
fn default_project_addr() -> String {
    if let Ok(val) = std::env::var("APKW_PROJECT_ADDR") {
        return val;
    }
    let cfg = load_cli_config();
...
fn default_build_addr() -> String {
    if let Ok(val) = std::env::var("APKW_BUILD_ADDR") {
        return val;
    }
    let cfg = load_cli_config();
...
fn default_observe_addr() -> String {
    if let Ok(val) = std::env::var("APKW_OBSERVE_ADDR") {
        return val;
    }
    let cfg = load_cli_config();
...
fn default_workflow_addr() -> String {
    if let Ok(val) = std::env::var("APKW_WORKFLOW_ADDR") {
        return val;
    }
    let cfg = load_cli_config();
        ...
        Cmd::Job { cmd } => match cmd {
            ...
            JobCmd::List {
                ...
                let filter = JobFilter {
                    ...
                    created_after: created_after
                        .map(|ms| apkw_proto::apkw::v1::Timestamp { unix_millis: ms }),
            ...
            JobCmd::History {
                ...
                let filter = JobHistoryFilter {
                    kinds,
                    after: after.map(|ms| apkw_proto::apkw::v1::Timestamp { unix_millis: ms }),
                    before: before.map(|ms| apkw_proto::apkw::v1::Timestamp { unix_millis: ms }),
                };
                let mut client = JobServiceClient::new(connect(&addr).await?);
                let resp = client
...
fn print_reload_result(name: &str, addr: &str, resp: &apkw_proto::apkw::v1::ReloadStateResponse) {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cli = Cli::parse();
    telemetry::init_with_env("apkw-cli", env!("CARGO_PKG_VERSION"));
