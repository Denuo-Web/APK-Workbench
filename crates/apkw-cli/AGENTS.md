...
- Implementation lives in crates/apkw-cli/src/main.rs.
...
- Job export helpers and shared data-dir defaults are provided by `apkw-util`.
- CLI config persists to `~/.local/share/apkw/state/cli-config.json` with last job selections.
- Telemetry is opt-in via env (APKW_TELEMETRY, APKW_TELEMETRY_CRASH) and emits cli.command.* events.
...
- State save/open commands block if the latest job is queued/running and serialize via ~/.local/share/apkw/state-ops; open reloads all services via ReloadState.
...
## Environment / config
- APKW_JOB_ADDR, APKW_TOOLCHAIN_ADDR, APKW_PROJECT_ADDR, APKW_BUILD_ADDR, APKW_TARGETS_ADDR,
  APKW_OBSERVE_ADDR, APKW_WORKFLOW_ADDR (defaults to 127.0.0.1 ports).
- APKW_TELEMETRY and APKW_TELEMETRY_CRASH enable usage/crash reporting (local spool).

## Prioritized TODO checklist by service
(Clients list includes UI and CLI items; some references below point to crates/apkw-ui.)
