...
fn log_reload_result(
    ...
    name: &str,
    addr: &str,
    resp: &apkw_proto::apkw::v1::ReloadStateResponse,
) {
...
async fn reload_job_state(
    addr: &str,
) -> Result<apkw_proto::apkw::v1::ReloadStateResponse, Box<dyn std::error::Error>> {
    let mut client = JobServiceClient::new(connect(addr).await?);
    let resp = client
...
pub(crate) async fn handle_command(
    ...
    match cmd {
        ...
        UiCommand::StateOpen {
            ...
            ui.send(AppEvent::Log {
                page: "settings",
                line: "Opening APKW state archive...\n".into(),
            })
            .ok();
            match latest_active_job(&cfg.job_addr).await {
                Ok(Some(job)) => {
        ...
        UiCommand::ResetAllState { cfg } => {
            let _ = cfg;
            let data_dir = apkw_util::data_dir();
            let dir_label = data_dir.display();
            ui.send(AppEvent::Log {
                page: "settings",
                line: format!("Resetting APKW state under {dir_label}\n"),
            })
            .ok();
