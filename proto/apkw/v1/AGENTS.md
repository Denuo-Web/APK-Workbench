# Proto Contracts Agent Notes (proto/apkw/v1)
...
## Recent changes
- Added ReloadStateRequest/ReloadStateResponse in common.proto and ReloadState RPCs to all services for on-demand state reloads.
...
## Code generation
- Build script: crates/apkw-proto/build.rs
- The build script uses a fixed list of protos and `tonic_build` to generate Rust types.
- If you add a new .proto file, you must:
  - add it to the `protos` list in crates/apkw-proto/build.rs
  - import it from other protos as needed
  - run `cargo build -p apkw-proto` (or a full workspace build)
- Generated Rust modules live under `apkw_proto::apkw::v1`.
...
## Versioning strategy
- Current API version is `apkw.v1`.
- Breaking changes should go into a new package (e.g., `apkw.v2`) while keeping v1 live until
...
- Update crates/apkw-proto/build.rs if adding files.
