...
fn job_state_file_path() -> PathBuf {
    apkw_util::state_file_path(JOB_STATE_FILE_NAME)
}
...
fn cleanup_bundles() -> io::Result<()> {
    ...
    let retention_days = read_env_u64(
        "APKW_OBSERVE_BUNDLE_RETENTION_DAYS",
        DEFAULT_BUNDLE_RETENTION_DAYS,
    );
    let max_bundles = read_env_usize("APKW_OBSERVE_BUNDLE_MAX", DEFAULT_BUNDLE_MAX);
    let tmp_hours = read_env_u64(
        "APKW_OBSERVE_TMP_RETENTION_HOURS",
...
fn add_state_file(items: &mut Vec<BundleItem>, file_name: &str) {
    items.push(BundleItem::File {
        source: apkw_util::state_file_path(file_name),
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    serve_grpc_with_telemetry(
        ...
        "observe",
        "APKW_OBSERVE_ADDR",
        apkw_util::DEFAULT_OBSERVE_ADDR,
