...
use apkw_proto::apkw::v1::{Id, KeyValue, Target, TargetKind};
...
pub(crate) fn adb_path() -> PathBuf {
    if let Ok(path) = std::env::var("APKW_ADB_PATH") {
...
pub(crate) fn adb_failure_message(err: &AdbFailure) -> String {
    match err {
        AdbFailure::NotFound => "adb not found (set APKW_ADB_PATH or ANDROID_SDK_ROOT)".into(),
...
pub(crate) fn adb_failure_status(err: AdbFailure) -> tonic::Status {
    match err {
        AdbFailure::NotFound => tonic::Status::failed_precondition(
            "adb not found (set APKW_ADB_PATH or ANDROID_SDK_ROOT)",
        ),
